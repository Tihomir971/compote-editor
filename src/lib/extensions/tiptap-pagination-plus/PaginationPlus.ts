...
const getPageConfig = (
	_storage: PaginationPlusStorage,
	_currentOptions: PaginationPlusOptions
): { config: PaginationPlusConfig; options: PaginationPlusOptions } => {
...
const getPageConfigFromOptions = (_currentOptions: PaginationPlusOptions): PaginationPlusConfig => {
	return {
		...
		contentMarginBottom:
			_currentOptions.contentMarginBottom ?? defaultPageConfig.contentMarginBottom,
			...
			new Plugin({
				...
				state: {
					...
					apply: (tr, oldDeco, oldState, newState) => {
						const { options: _currentOptions, config: pageConfig } = getPageConfig(
							storage,
							this.options
						);
						...
						const getNewDecoration = () => {
							...
							const headerHeight =
								'headerHeight' in this.storage ? this.storage.headerHeight : new Map();
							const footerHeight =
								'footerHeight' in this.storage ? this.storage.footerHeight : new Map();
	...
	if (paginationElement) {
		...
		if (lastElementOfEditor && lastPageBreak) {
			...
			if (lastPageGap > 0) {
				...
				const allBreaksAfterLastElement = Array.from(
					paginationElement.querySelectorAll('.breaker')
				);
