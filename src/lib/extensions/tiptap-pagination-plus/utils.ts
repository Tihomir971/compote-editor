import type { PaginationPlusOptions } from './PaginationPlus';
...
export function deepEqualIterative(a: unknown, b: unknown): boolean {
	...
	const stack: Array<{ x: Record<string, unknown>; y: Record<string, unknown> }> = [
		{ x: a as Record<string, unknown>, y: b as Record<string, unknown> }
	];
	...
	while (stack.length) {
		...
		for (const key of xKeys) {
			...
			if (typeof xVal === 'object' && typeof yVal === 'object') {
				stack.push({ x: xVal as Record<string, unknown>, y: yVal as Record<string, unknown> });
			} else {
				// Primitive compare
